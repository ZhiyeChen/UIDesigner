...
#define MSGDEF(x) if(uMsg==x) return _T(#x)
	...
	{
		if (iSize < 0 || iSize >= (_UI16_MAX / sizeof(LPVOID)))
			return;
		Empty();
		size_t allocatedsize = iSize * sizeof(LPVOID);
		m_ppVoid = static_cast<LPVOID*>(malloc(allocatedsize));
		::ZeroMemory(m_ppVoid, allocatedsize);
		m_nAllocated = iSize;
		m_nCount = iSize;
	}
	...
	bool CStdValArray::Add(LPCVOID pData)
	{
		...
		memcpy_s(m_pVoid + ((m_nCount - 1) * m_iElementSize), m_iElementSize, pData, m_iElementSize);
		return true;
	}
	...
	bool CStdValArray::Remove(int iIndex)
	{
		if (iIndex < 0 || iIndex >= m_nCount) return false;
		if (iIndex < --m_nCount)
			memcpy_s(m_pVoid + (iIndex * m_iElementSize), (m_nCount - iIndex) * m_iElementSize, m_pVoid + ((iIndex + 1) * m_iElementSize), (m_nCount - iIndex) * m_iElementSize);
	...
	CStdStringPtrMap::CStdStringPtrMap(int nSize) : m_nCount(0)
	{
		...
		if (nSize >= (_UI16_MAX / sizeof(TITEM*)))		 //integer overflow protection
			return;
		m_nBuckets = nSize;

...
namespace DuiLib {
	...
	CControlUI::CControlUI() :
		...
		m_bMenuUsed(false),
		m_bVisible(true),
		m_bShowVirtualKeyboard(false),//ZOOM-21796
		m_bInternVisible(true),
		m_bFocused(false),
		m_bEnabled(true),
		m_bMouseEnabled(true),
		m_bKeyboardEnabled(true),
		m_bSupportTabStop(false),
		m_bFloat(false),
		m_bSetPos(false),
		m_chShortcut('\0'),
		m_pTag(NULL),
		...
		m_nTooltipWidth(300),
		m_dwFocusFrameColor(0x333333),
		//m_pAccProxy(NULL),
		m_bSel(false),
		m_bTransparent(false),
		customize_acc_role_(0)
	{
		m_cXY.cx = m_cXY.cy = 0;
		...
		::ZeroMemory(&m_rcFocusPadding, sizeof(RECT));
		::ZeroMemory(&m_sizeFocusRound, sizeof(SIZE));
	}
	...
	{
		int newSize = nSize;
		if (newSize == m_nBorderSize)
			return;
		m_nBorderSize = newSize;
		Invalidate();
	}
	void CControlUI::SetBorderSize(RECT rc)
	{
		...
		if (newRc.left == m_rcBorderSize.left &&
			newRc.top == m_rcBorderSize.top &&
			newRc.right == m_rcBorderSize.right &&
			newRc.bottom == m_rcBorderSize.bottom)
			return;
		m_rcBorderSize = newRc;
	...
	void CControlUI::AfterSetPos()
	{
		if (OnSized) OnSized(this);
	}
	//************************************
	...
	void CControlUI::SetLeftBorderSize(int nSize)
	{
		...
		if (newSize == m_rcBorderSize.left)
			return;
		m_rcBorderSize.left = newSize;
	...
	void CControlUI::SetTopBorderSize(int nSize)
	{
		...
		if (newSize == m_rcBorderSize.top)
			return;
		m_rcBorderSize.top = newSize;
	...
	void CControlUI::SetRightBorderSize(int nSize)
	{
		...
		if (newSize == m_rcBorderSize.right)
			return;
		m_rcBorderSize.right = newSize;
	...
	void CControlUI::SetBottomBorderSize(int nSize)
	{
		...
		if (newSize == m_rcBorderSize.bottom)
			return;
		m_rcBorderSize.bottom = newSize;
	...
	void CControlUI::SetFocusPadding(const RECT& rcPadding)
	{
		m_rcFocusPadding = rcPadding;
	}

	RECT CControlUI::GetFocusPadding() const
	{
		return m_rcFocusPadding;
	}

	void CControlUI::SetFocusRound(const SIZE& sizeRound)
	{
		m_sizeFocusRound = sizeRound;
	}

	SIZE CControlUI::GetFocusRound() const
	{
		return m_sizeFocusRound;
	}

	void CControlUI::SetFocusFrameColor(DWORD dwColor)
	{
		m_dwFocusFrameColor = dwColor;
	}

	DWORD CControlUI::GetFocusFrameColor() const
	{
		return m_dwFocusFrameColor;
	}
	...
	bool CControlUI::IsNativeVisible() const
	{
		return m_bVisible;
	}
	bool CControlUI::IsSupportTabStop() const
	{
		return m_bSupportTabStop;
	}

	void CControlUI::SetSupportTabStop(bool bSupport)
	{
		m_bSupportTabStop = bSupport;
	}
	bool CControlUI::DetachFromManager()
	{
		if (m_pManager)
			m_pManager->ReapObjects(this);

		SetManager(NULL, NULL, false);
		NeedUpdate();

		return true;
	}
	bool CControlUI::AttachToManager(CPaintManagerUI * pm, CControlUI * parent) {
		if (m_pManager != NULL)
			return false;

		SetManager(pm, parent, false);
		NeedUpdate();

		return true;
	}
	bool CControlUI::IsMsgFocusControlWant(UINT nMsg, WPARAM wParam, LPARAM lParam)
	{
		return false;
	}
	...
	CStdString CControlUI::GetVirtualWnd() const
	{
		CStdString str;
